# This is Section 6.3 in the manuscript #
...
run_sim_es <- function(i, p) {
  ...
  cohorts <- data6 %>% 
    # drop never treateds, and also 2015 when everyone is treated
...
total = cs_es + stacked_es
total

ggsave(total, filename = here::here("Figs_Tables", "new_did_sims.png"), 
       dpi = 500,
       width = 8, height = 4)

write.csv(stacked_data, file = "stacked_data.csv")
